...
class GraphService:
    ...
    def vector_search(self, embedding, max_results: int = 20) -> List[Dict[str, Any]]:
        ...
        """
        logger.info(f"Vector search with max_results={max_results}")
        ...
        # Check if the client supports vector search
        if not hasattr(client, "vector_search"):
            logger.warning(f"Graph database {self.graph_db_type} does not support vector search")
            return []

        # Perform vector search
        try:
            results = client.vector_search(embedding, max_results=max_results)
