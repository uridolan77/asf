...
a high-performance, in-memory graph database. The client implements methods
for storing and querying medical knowledge graphs, including articles,
concepts, authors, and their relationships.
...
Features:
- Connection management with Memgraph database
...
# numpy is used for vector operations in search_articles_by_embedding
import numpy as np
...
class MemgraphClient:
    ...
    in Memgraph. It implements the singleton pattern to ensure a single connection
    ...
    def __init__(self):
        ...
        application settings. It sets up the host and port for the Memgraph
        ...
        - MEMGRAPH_HOST: Hostname or IP address of the Memgraph server
        - MEMGRAPH_PORT: Port number for the Memgraph server
        """
    ...
    def connect(self) -> bool:
        ...
        This method establishes a connection to the Memgraph database using the
        host and port specified during initialization. If a connection already
        exists, it is closed before establishing a new one.
        ...
        try:
            self.connection = pymemgraph.connect(
                host=self.host,
                port=self.port
            )
            # Test the connection
            ...
            return True
        except pymemgraph.MemgraphConnectionError as e:
            logger.error(f"Failed to connect to Memgraph: {str(e)}")
            self.connection = None
            return False
        except Exception as e:
            logger.error(f"Unexpected error connecting to Memgraph: {str(e)}")
            self.connection = None
            return False
    def execute_query(self, query: str, params: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        """
        Execute a Cypher query against the Memgraph database.

        This method executes a Cypher query with optional parameters against the
        Memgraph database. It handles connection management, query execution,
        ...
        Raises:
            pymemgraph.MemgraphConnectionError: If connection to the database fails
            pymemgraph.MemgraphQueryError: If the query execution fails
            Exception: For any other errors
        """
        if not self.connection:
            connected = self.connect()
            if not connected:
                raise pymemgraph.MemgraphConnectionError("Failed to connect to Memgraph database")

        try:
            result = self.connection.execute_query(query, params or {})
            return [dict(record) for record in result]
        except pymemgraph.MemgraphQueryError as e:
    ...
    def create_concept(self, concept: Dict[str, Any]) -> str:
        ...
        This method creates a new concept node in the Memgraph database with the
    ...
    def create_article_concept_relationship(self, pmid: str, cui: str, relationship_type: str = "MENTIONS", properties: Dict[str, Any] = None) -> None:
        ...
        Memgraph database. The relationship type can be specified, with MENTIONS being
    ...
    def create_concept_relationship(self, cui1: str, cui2: str, relationship_type: str, properties: Dict[str, Any] = None) -> None:
        ...
        This method creates a relationship between two concepts in the Memgraph database.
    ...
    def create_contradiction(self, pmid1: str, pmid2: str, contradiction_score: float, confidence: float, topic: str = None, explanation: str = None) -> None:
        ...
        Memgraph database. The relationship includes properties such as contradiction score,
    ...
    def get_article(self, pmid: str) -> Dict[str, Any]:
        ...
        This method retrieves an article from the Memgraph database by its PMID.
    ...
    def get_concept(self, cui: str) -> Dict[str, Any]:
        ...
        This method retrieves a concept from the Memgraph database by its CUI
    ...
    def get_concepts_by_article(self, pmid: str) -> List[Dict[str, Any]]:
        ...
        from the Memgraph database. It includes the frequency of mentions if available.
    ...
    def get_articles_by_concept(self, cui: str, max_results: int = 100) -> List[Dict[str, Any]]:
        ...
        Memgraph database. It includes the frequency of mentions if available.
    ...
    def get_contradictions(self, pmid: str = None, max_results: int = 100) -> List[Dict[str, Any]]:
        ...
        This method retrieves contradictions between articles from the Memgraph database.
    ...
    def get_related_concepts(self, cui: str, relationship_type: str = None, max_results: int = 100) -> List[Dict[str, Any]]:
        ...
        Memgraph database. If a relationship type is provided, it retrieves only concepts
    ...
    def search_articles_by_embedding(self, embedding: List[float], max_results: int = 20, similarity_threshold: float = 0.5) -> List[Dict[str, Any]]:
        ...
        using cosine similarity. It requires the Memgraph Graph Data Science (MGDS) library.
    ...
    def create_article(self, article: Dict[str, Any]) -> str:
        ...
        This method creates a new article node in the Memgraph database with the
    ...
    def create_author(self, pmid: str, author: str) -> None:
        ...
        This method creates a new author node in the Memgraph database and establishes
