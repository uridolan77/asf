...
class GenerativeReplay(ReplayStrategy):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Initialize generation cache if enabled
        self.cache = None
        if use_cache:
            self.cache = GenerationCache(cache_dir=cache_dir)
    ...
    def _generate_texts(self, prompt: str, num_examples: int, gen_params: Dict[str, Any]) -> List[str]:
        ...
        # Use batch generation if possible
    ...
    def _get_all_generated_examples(self) -> List[Dict[str, Any]]:
        """
    ...
    def after_training(self, task_id: str, **kwargs):
        ...
        For Generative Replay, this includes updating the task history
        and clearing the cache if needed.
        ...
        if self.cache and len(self.generated_examples) > 10:
            logger.info("Clearing generation cache to save memory")
            self.cache.clear()
