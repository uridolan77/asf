"""
Enhanced Elastic Weight Consolidation (EWC) for CL-PEFT.

This module implements improved EWC strategies for mitigating catastrophic forgetting
in sequential fine-tuning of LLMs with PEFT, including:
- Standard EWC: Original implementation from Kirkpatrick et al.
...
class EnhancedEWC(CLStrategy):
    """
    Enhanced Elastic Weight Consolidation (EWC) strategy for CL-PEFT.
    
    This implementation includes several improvements over the standard EWC:
    - Support for different EWC variants (standard, online, synaptic intelligence)
    - Memory-efficient Fisher computation with gradient accumulation
    - Support for different Fisher estimation methods
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize the Enhanced EWC strategy.
        ...
        Args:
            ...
            ewc_variant: EWC variant to use ("standard", "online", or "synaptic")
            online_gamma: Decay factor for Online EWC (0 to 1)
            memory_efficient: Whether to use memory-efficient Fisher computation
        ...
        # Batch accumulation for memory-efficient Fisher computation
        self.fisher_accumulation = {}   # {param_name -> accumulated_fisher}
    ...
    def before_training(self, task_id: str, **kwargs):
        ...
        For EWC, this initializes tracking structures and, for Synaptic Intelligence,
        ...
        logger.info(f"Starting training on task {task_id} with Enhanced EWC ({self.ewc_variant})")
        
        # For Synaptic Intelligence, store initial parameter values
        if self.ewc_variant == "synaptic":
            self.param_prev = {}
            self.param_delta = {}
            ...
            for name, param in self.model.named_parameters():
                if param.requires_grad:
                    ...
                    # Initialize omega if not exists
    ...
    def modify_loss(
        ...
        **kwargs
        ...
        """
        # If no previous tasks or not using EWC, return original loss
        if (self.ewc_variant == "standard" and not self.parameter_importance) or \
           (self.ewc_variant == "online" and not self.online_fisher) or \
           (self.ewc_variant == "synaptic" and not self.omega):
            return loss
        ...
        for name, param in model.named_parameters():
            ...
            if self.ewc_variant == "standard":
                # Standard EWC: separate Fisher matrix for each task
            ...
            elif self.ewc_variant == "online":
                # Online EWC: single Fisher matrix updated with decay
                if name in self.online_fisher and name in self.online_optimal:
            ...
            elif self.ewc_variant == "synaptic":
                # Synaptic Intelligence: use accumulated omega values
                if name in self.omega:
                    # Get omega values for this parameter
                    omega = self.omega[name]
                    
                    # If this is a new parameter, skip
                    if name not in self.param_prev:
                        continue
                    ...
                    ewc_loss += (omega * (param - prev_value).pow(2)).sum()
    ...
    def after_training(self, task_id: str, train_dataset=None, **kwargs):
        ...
        Args:
            ...
            **kwargs: Additional arguments
        """
        logger.info(f"Computing importance metrics for task {task_id}")
        
        # Store current parameter values as optimal for this task
        if self.ewc_variant in ["standard", "online"]:
            # For standard and online EWC, store optimal parameter values
            if self.ewc_variant == "standard":
            ...
            if train_dataset is not None:
                if self.ewc_variant == "standard":
                    ...
                    # For Online EWC, update the single Fisher matrix with decay
            ...
            # For Synaptic Intelligence, update omega based on parameter changes
            for name, param in self.model.named_parameters():
                if param.requires_grad and name in self.param_prev:
                    ...
                    if name in self.param_delta and torch.any(delta != 0):
                        ...
                        omega_update = self.param_delta[name] / (delta.pow(2) + 1e-7)
    ...
    def _compute_fisher_information(self, dataset) -> Dict[str, torch.Tensor]:
        ...
        # Reset accumulation structures if using memory-efficient computation
        if self.memory_efficient:
            self.fisher_accumulation = {}
            self.accumulation_count = 0
            ...
            for name, param in self.model.named_parameters():
                if param.requires_grad and param.grad is not None:
                    if self.memory_efficient:
                        ...
                        if name not in self.fisher_accumulation:
                            self.fisher_accumulation[name] = param.grad.data.pow(2).detach()
                        else:
                            self.fisher_accumulation[name] += param.grad.data.pow(2).detach()
                        self.accumulation_count += 1
                        
                        # Periodically update fisher_information to free memory
                        if self.accumulation_count % 10 == 0:
                            for acc_name, acc_fisher in self.fisher_accumulation.items():
                                fisher_information[acc_name] += acc_fisher / 10
                                self.fisher_accumulation[acc_name] = torch.zeros_like(acc_fisher)
                    else:
                        # Direct accumulation
                        fisher_information[name] += param.grad.data.pow(2).detach()
        
        # Final accumulation for memory-efficient computation
        if self.memory_efficient and self.accumulation_count > 0:
            remaining_count = self.accumulation_count % 10
            if remaining_count > 0:
                for acc_name, acc_fisher in self.fisher_accumulation.items():
                    fisher_information[acc_name] += acc_fisher / remaining_count
    ...
    def _store_importance_metrics(self, task_id):
        ...
        if self.ewc_variant == "standard" and task_id in self.parameter_importance:
        ...
        elif self.ewc_variant == "online" and self.online_fisher:
            for name, importance in self.online_fisher.items():
                # Compute a scalar importance score (mean of absolute values)
                importance_score = importance.abs().mean().item()
                self.importance_history[task_id][name] = importance_score
        
        elif self.ewc_variant == "synaptic" and self.omega:
    ...
    def modify_gradients(self, **kwargs):
        ...
        For standard and online EWC, this is a no-op as the regularization is applied in the loss.
        For Synaptic Intelligence, this accumulates parameter-gradient products.
        
        Args:
            **kwargs: Additional arguments
        """
        # For Synaptic Intelligence, accumulate parameter-gradient products
        if self.ewc_variant == "synaptic":
            for name, param in self.model.named_parameters():
                if param.requires_grad and param.grad is not None:
        ...
        # No gradient modification needed for standard and online EWC
        pass
