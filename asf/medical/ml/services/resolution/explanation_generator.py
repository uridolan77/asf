...
def generate_explanation(
    ...
    if recommendation == RecommendationType.FAVOR_CLAIM1:
        ...
        explanation["summary"] = f"The contradiction is resolved in favor of the second claim: '{claim2}'. "
        explanation["summary"] += f"Confidence: {confidence}."
    elif recommendation == RecommendationType.CONDITIONAL:
        explanation["summary"] = f"The contradiction resolution is conditional: {recommended_claim} "
        explanation["summary"] += f"Confidence: {confidence}."
    elif recommendation == RecommendationType.FURTHER_RESEARCH:
        explanation["summary"] = f"Further research is needed to resolve this contradiction. {recommended_claim} "
        explanation["summary"] += f"Confidence: {confidence}."
    else:  # INCONCLUSIVE
    ...
    if strategy == ResolutionStrategy.EVIDENCE_HIERARCHY:
        ...
        explanation["detailed_reasoning"] = generate_sample_size_explanation(resolution)
    elif strategy == ResolutionStrategy.RECENCY_PREFERENCE:
        explanation["detailed_reasoning"] = generate_recency_explanation(resolution)
    elif strategy == ResolutionStrategy.POPULATION_SPECIFICITY:
        explanation["detailed_reasoning"] = generate_population_explanation(resolution)
    elif strategy == ResolutionStrategy.METHODOLOGICAL_QUALITY:
        explanation["detailed_reasoning"] = generate_methodological_explanation(resolution)
    elif strategy == ResolutionStrategy.STATISTICAL_SIGNIFICANCE:
        explanation["detailed_reasoning"] = generate_statistical_explanation(resolution)
    elif strategy == ResolutionStrategy.COMBINED_EVIDENCE:
        explanation["detailed_reasoning"] = generate_combined_evidence_explanation(resolution)
    else:
...
def generate_recency_explanation(resolution: Dict[str, Any]) -> str:
    ...
    current_year = recency_comparison.get("current_year", 0)
...
def generate_methodological_explanation(resolution: Dict[str, Any]) -> str:
    ...
    differences = methodological_comparison.get("differences", [])
...
def generate_combined_evidence_explanation(resolution: Dict[str, Any]) -> str:
    ...
    strategy_results = combined_evidence.get("strategy_results", {})
...
def generate_clinical_implications(contradiction: Dict[str, Any], resolution: Dict[str, Any], clinical_significance: str) -> str:
    """
...
def generate_limitations(contradiction: Dict[str, Any], resolution: Dict[str, Any], strategy: ResolutionStrategy) -> str:
    """
...
def generate_references(contradiction: Dict[str, Any], resolution: Dict[str, Any], strategy: ResolutionStrategy) -> List[str]:
    """
