...
class MockClient(BaseProvider):
    ...
    def __init__(self, provider_id: str, provider_config: ProviderConfig, gateway_config: GatewayConfig = None):
        """Initialize the Mock Client."""
        # Ensure provider_id is set in provider_config
        if not hasattr(provider_config, 'provider_id') or not provider_config.provider_id:
            # Create a new ProviderConfig with provider_id set
            from copy import deepcopy
            config_dict = provider_config.model_dump()
            config_dict['provider_id'] = provider_id
