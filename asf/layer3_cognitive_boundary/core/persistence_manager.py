...
class SemanticPersistenceManager:
    ...
    async def patch_network(self, node_modifications: Dict[str, Any] = None, 
                          relation_modifications: Dict[str, Any] = None):
        """
        Apply patches to semantic network.
        
        Args:
            node_modifications: Dict of node IDs to modification operations
            relation_modifications: Dict of relation IDs to modification operations
            
        Returns:
            Dict with patch statistics
        """
        node_modifications = node_modifications or {}
        relation_modifications = relation_modifications or {}
        ...
        for node_id, modification in node_modifications.items():
            try:
                # Get node if it exists
                node = await self.semantic_network.get_node(node_id)
                
                if modification.get('operation') == 'delete':
                    # Delete node (not directly supported by semantic network)
                    if node_id in self.semantic_network.nodes:
                        del self.semantic_network.nodes[node_id]
                        if node_id in self.semantic_network.node_index:
                            idx = self.semantic_network.node_index[node_id]
                            del self.semantic_network.node_index[node_id]
                            if idx in self.semantic_network.index_to_node:
                                del self.semantic_network.index_to_node[idx]
                        
                        # Mark for rebuild
                        self.semantic_network.rebuild_needed = True
                        patched_nodes += 1
                        
                elif modification.get('operation') == 'update':
                    if node is not None:
                        # Update node properties
                        if 'properties' in modification:
                        ...
                        # Update other fields
                        for field in ['label', 'confidence', 'metadata']:
                            if field in modification:
                                setattr(node, field, modification[field])
                                
                        # Update node in network
                        await self.semantic_network.add_node(node)
                        patched_nodes += 1
                        
                elif modification.get('operation') == 'create':
                    # Create new node
                    new_node = SemanticNode(
                        id=node_id,
                        label=modification.get('label', f"Node_{node_id[-6:]}"),
                        node_type=modification.get('node_type', SemanticNodeType.CONCEPT.value),
                        properties=modification.get('properties', {}),
                        confidence=modification.get('confidence', 0.7)
                    )
                    
                    # Add embeddings if provided
                    if 'embeddings' in modification:
                        new_node.embeddings = np.array(modification['embeddings'], dtype=np.float32)
                        
                    # Add to network
                    await self.semantic_network.add_node(new_node)
                    patched_nodes += 1
            except Exception as e:
                self.logger.error(f"Error patching node {node_id}: {e}")
                errors += 1
                
        # Apply relation modifications
        for relation_id, modification in relation_modifications.items():
            try:
                # Get relation if it exists
                relation = await self.semantic_network.get_relation(relation_id)
                
                if modification.get('operation') == 'delete':
                    # Delete relation (not directly supported by semantic network)
                    if relation_id in self.semantic_network.relations:
                        relation = self.semantic_network.relations[relation_id]
                        ...
                        if source_id in self.semantic_network.relation_index['source']:
                            if relation_id in self.semantic_network.relation_index['source'][source_id]:
                                self.semantic_network.relation_index['source'][source_i