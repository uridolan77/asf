...
import { useMedicalClients, MedicalClient, ClientConfig } from '../../hooks/useMedicalClients';
...
interface ClientConfigDialogProps {
  ...
  onUpdate: (clientId: string, config: ClientConfig) => void;
}
 ...
 */
const ClientConfigDialog: React.FC<ClientConfigDialogProps> = ({ open, onClose, client, onUpdate }) => {
  // Form state
  const [config, setConfig] = useState<ClientConfig>({});
  const [showApiKey, setShowApiKey] = useState<boolean>(false);
  const [errors, setErrors] = useState<FormErrors>({});
  
  // Get the update client config mutation
  const { updateClientConfig, useMockData } = useMedicalClients();
  const { mutate: updateConfig, isPending: isUpdating } = updateClientConfig(client?.id || '');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Numeric validation
    if (config.timeout && (isNaN(Number(config.timeout)) || Number(config.timeout) <= 0)) {
      newErrors.timeout = 'Please enter a positive number';
    }
    
    if (config.max_retries && (isNaN(Number(config.max_retries)) || Number(config.max_retries) <= 0)) {
      newErrors.max_retries = 'Please enter a positive number';
    }
    
    if (config.cache_ttl && (isNaN(Number(config.cache_ttl)) || Number(config.cache_ttl) <= 0)) {
    ...
    // Submit form
    if (useMockData) {
      // If using mock data, just call the onUpdate callback
      onUpdate(client.id, config);
      onClose();
    } else {
            ...
            <Grid item xs={12}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  label="Access Mode"
                  onChange={(e: SelectChangeEvent) => handleChange('access_mode', e.target.value)}
                >
            ...
            <Grid item xs={12}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  label="Edition"
                  onChange={(e: SelectChangeEvent) => handleChange('edition', e.target.value)}
                >
      ...
      <form onSubmit={handleSubmit}>
        ...
        <DialogActions>
          ...
          <Button 
            ...
            variant="contained" 
            color="primary"
            startIcon={isUpdating ? <ButtonLoader size={20} /> : <SaveIcon />}
            disabled={isUpdating}
          >
