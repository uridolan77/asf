"""
Clean version of the backend server with all observability components disabled.

This script is a modified version of run.py that explicitly disables all
observability components including tracing, metrics, and Prometheus exporters.
It also applies patches to prevent these components from initializing.
"""
...
# Monkey patch observability modules
def monkey_patch_observability():
    """
    Monkey patch observability modules to prevent them from initializing.
    """
    # Create dummy classes for observability components
    ...
    # Patch modules if they're imported
    modules_to_patch = {
        ...
        'asf.medical.llm_gateway.observability.prometheus': {
            ...
            'get_prometheus_exporter': lambda: DummyPrometheusExporter(),
            'configure_prometheus_exporter': lambda *args, **kwargs: DummyPrometheusExporter(),
        },
    }
        ...
        if module_name in sys.modules:
            for attr_name, replacement in patches.items():
                try:
                    ...
                    print(f"Failed to patch {module_name}.{attr_name}: {e}")

# Try to apply monkey patches
try:
    monkey_patch_observability()
    print("Successfully applied monkey patches to observability modules")
except Exception as e:
