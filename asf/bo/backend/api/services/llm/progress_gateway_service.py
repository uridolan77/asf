"""
Progress-tracking LLM Gateway service.
...
class ProgressGatewayService:
    """
    Service for interacting with the LLM Gateway with progress tracking.
    ...
    def __init__(self):
        """Initialize the progress gateway service."""
        self._client = None
        self._config = None
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            progress_factory = ProgressTrackingFactory()
            
            # Create progress-tracking LLM client
            self._client = progress_factory.create_client(
                config=gateway_config,
                provider_factory=provider_factory
            )
            
            logger.info(f"Progress-tracking LLM Gateway client initialized with gateway ID: {gateway_config.gateway_id}")
        except Exception as e:
            logger.error(f"Failed to initialize progress-tracking LLM Gateway client: {str(e)}")
            self._client = None
...
# Singleton instance
_progress_gateway_service = None


def get_progress_gateway_service() -> ProgressGatewayService:
    """
    Get the singleton instance of the progress gateway service.
    
    Returns:
        Progress gateway service
    """
    global _progress_gateway_service
    if _progress_gateway_service is None:
        _progress_gateway_service = ProgressGatewayService()
    return _progress_gateway_service
