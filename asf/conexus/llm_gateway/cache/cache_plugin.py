"""
Caching Plugin for Conexus LLM Gateway.
...
from asf.conexus.llm_gateway.core.plugins import BasePlugin, PluginCategory, PluginEventType
...
class CachingPlugin(BasePlugin):
    ...
    async def on_event(self, event_type: PluginEventType, payload: Any) -> Any:
        ...
        if event_type == PluginEventType.REQUEST_START and isinstance(payload, dict) and "request" in payload:
        ...
        elif event_type == PluginEventType.RESPONSE_END and isinstance(payload, dict) and "request" in payload and "response" in payload:
            request = payload["request"]
...
def register_plugin():
    """Register the caching plugin with the plugin registry."""
    from asf.conexus.llm_gateway.core.plugins import get_plugin_registry
    registry = get_plugin_registry()
    registry.register_plugin_class(CachingPlugin)
