...
logger = logging.getLogger("conexus.llm_gateway.resilience.retry")
...
class RetryPolicy:
    ...
    def __init__(
        ...
        logger.debug(
            f"[{name}] Initialized retry policy with max_retries={max_retries}, "
            f"base_delay={base_delay}, max_delay={max_delay}, jitter_factor={jitter_factor}"
        )
    ...
    def is_retryable_error(self, error: Exception) -> bool:
        ...
        if self.retry_predicate is not None:
            try:
                ...
                logger.warning(
                    f"[{self.name}] Error in custom retry predicate: {str(e)}"
                )
        
        # Check for errors with error_code attribute
        if hasattr(error, 'error_code'):
            return error.error_code in self.retry_codes
    ...
    def record_retry_attempt(self, successful: bool) -> None:
        ...
        logger.debug(
            f"[{self.name}] Recorded retry attempt, successful={successful}, "
            f"total_attempts={self.retry_attempts}, success_rate={success_rate}"
        )
...
async def retry_async(
    ...
    for attempt in range(retry_policy.max_retries + 1):
        try:
            ...
            if attempt > 0:
                ...
                if on_retry:
                    try:
                        ...
                        logger.warning(
                            f"[{retry_policy.name}] Error in retry callback: {str(e)}"
                        )
                
                # Log retry attempt
                logger.info(
                    f"[{retry_policy.name}] Retrying function call: attempt={attempt}, "
                    f"max_attempts={retry_policy.max_retries + 1}, "
                    f"delay={delay}, error={str(last_exception)}"
                )
            ...
            log_method(
                f"[{retry_policy.name}] Function call failed: attempt={attempt + 1}, "
                f"max_attempts={retry_policy.max_retries + 1}, "
