...
This module provides rate limiting capabilities for the LLM Gateway,
...
logger = logging.getLogger("conexus.llm_gateway.resilience.rate_limiter")
...
class RateLimiter:
    ...
    This class provides rate limiting for the LLM Gateway,
    with support for token bucket, sliding window, and adaptive rate limiting.
    """
    ...
    def __init__(
        ...
        logger.info(
            f"[{name}] Initializing rate limiter with strategy={config.strategy.value}, "
    ...
    async def wait(self) -> None:
        ...
        while True:
            ...
            if wait_time > 0:
                logger.debug(f"[{self.name}] Rate limited, waiting for {wait_time:.2f} seconds")
