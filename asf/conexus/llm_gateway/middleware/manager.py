...
from asf.conexus.llm_gateway.core.models import (
    ...
    GatewayConfig, ProviderConfig, RoutingError, UsageStats, ToolUseRequest
)
from asf.conexus.llm_gateway.core.factory import ProviderFactory, ProviderFactoryError
from asf.conexus.llm_gateway.middleware.factory import InterventionFactory, InterventionFactoryError
from asf.conexus.llm_gateway.middleware.base import BaseIntervention, InterventionHookType
...
class InterventionManager:
    ...
    def __init__(self, provider_factory: ProviderFactory, gateway_config: GatewayConfig, db: Optional[Session] = None):
    ...
    def _get_enabled_interventions(self, context: InterventionContext, hook: InterventionHookType) -> List[str]:
        """
        Get the list of enabled interventions for a specific hook type.
        
        Args:
            context: The intervention context
            hook: The hook type ("pre", "post", "stream")
            
        Returns:
            List of enabled intervention names for the specified hook
        """
        if hook == "pre":
            return context.intervention_config.enabled_pre_interventions
        elif hook == "post":
            return context.intervention_config.enabled_post_interventions
        elif hook == "stream":
            # For stream, combine both pre and post interventions that support streaming
            # This is a simplification - in a real system, you might have a separate list for stream interventions
            return list(set(context.intervention_config.enabled_pre_interventions + 
                           context.intervention_config.enabled_post_interventions))
        else:
            # For combined hooks like "pre_post", split and process each part
            if "_" in hook:
                parts = hook.split("_")
                result = []
                for part in parts:
                    if part in ["pre", "post", "stream"]:
                        result.extend(self._get_enabled_interventions(context, part))
    ...
    async def _select_provider_with_failover(
        ...
        # First look for the provider config in the DB, not in the gateway configuration
    ...
    def _get_provider_config(self, provider_id: str) -> ProviderConfig:
         ...
         First tries to get the configuration from the database, then falls back to the gateway configuration.
         """
         # Try to get the configuration from the database first
         if self.db:
             try:
                from asf.medical.llm_gateway.core.config_loader import ConfigLoader
                config_loader = ConfigLoader(self.db)
                db_config = config_loader.load_config(provider_id)
                
                # Check if provider config exists in the database
                if db_config and "additional_config" in db_config and "providers" in db_config["additional_config"]:
                    providers_dict = db_config["additional_config"]["providers"]
                    if provider_id in providers_dict:
                        provider_data = providers_dict[provider_id]
                        # Convert to ProviderConfig if it's a dict
                        if isinstance(provider_data, dict):
                            provider_data["provider_id"] = provider_id  # Ensure provider_id is set
                            return ProviderConfig(**provider_data)
                        return provider_data
             except Exception as e:
                 logger.warning(f"Failed to get provider config from database for '{provider_id}': {e}")
                 # Fall back to gateway configuration
         
         # Fall back to gateway configuration
         provider_configs = self.gateway_config.providers # Use structured field
         config_data = provider_configs.get(provider_id)
         if not config_data:
              # Try to load the configuration from the YAML file directly
              try:
                  from asf.medical.llm_gateway.core.config_loader import ConfigLoader
                  config_loader = ConfigLoader()
                  yaml_config = config_loader.load_from_yaml()
                  
                  # Check if provider config exists in the YAML file
                  if yaml_config and "additional_config" in yaml_config and "providers" in yaml_config["additional_config"]:
                      providers_dict = yaml_config["additional_config"]["providers"]
                      if provider_id in providers_dict:
                          provider_data = providers_dict[provider_id]
                          # Convert to ProviderConfig if it's a dict
                          if isinstance(provider_data, dict):
                  ...
                  logger.warning(f"Failed to get provider config from YAML for '{provider_id}': {e}")
              
              # If we still can't find the provider configuration, raise an error
    ...
    def _get_provider_id_for_model(self, model_identifier: str, exclude_provider_id: Optional[str] = None) -> str:
        ...
        First tries to get the mapping from the database, then falls back to the gateway configuration.
        ...
        if self.db:
            try:
                from asf.medical.llm_gateway.core.config_loader import ConfigLoader
                config_loader = ConfigLoader(self.db)
                db_config = config_loader.load_config(use_db=True)
                
                # Check if model-provider mapping exists in the database
                if db_config and "model_provider_mapping" in db_config:
                    db_mapping = db_config["model_provider_mapping"]
                    if model_identifier in db_mapping:
                        explicit_provider_id = db_mapping[model_identifier]
                        if explicit_provider_id != exclude_provider_id:
                            logger.debug(f"Routing model '{model_identifier}' to provider '{explicit_provider_id}' via DB mapping.")
                            # We'll verify this provider is configured later
                            return explicit_provider_id
                        else:
                            logger.warning(f"DB-mapped provider '{exclude_provider_id}' for model '{model_identifier}' is excluded. Looking for alternatives.")
            except Exception as e:
                logger.warning(f"Failed to get model-provider mapping from database: {e}")
                # Fall back to gateway configuration
        
        # 2. Check explicit mapping in gateway config
        ...
        if model_identifier in model_mapping:
             ...
             if explicit_provider_id != exclude_provider_id:
                  ...
                  if explicit_provider_id not in self.gateway_config.providers:
                      ...
                      try:
                          provider_config = self._get_provider_config(explicit_provider_id)
                          return explicit_provider_id
                      except ValueError:
        ...
        # 3. Find all configured providers supporting the model
        ...
        if not candidate_providers:
             # 4. Try to get default provider from database
             db_default_provider = None
             if self.db:
                 try:
                     from asf.medical.llm_gateway.core.config_loader import ConfigLoader
                     config_loader = ConfigLoader(self.db)
                     ...
                     if db_config and "default_provider" in db_config:
                         db_default_provider = db_config["default_provider"]
                 except Exception as e:
                     logger.warning(f"Failed to get default provider from database: {e}")
             
             # 5. Use database default provider if available
             if db_default_provider and db_default_provider != exclude_provider_id:
                 logger.warning(f"Model '{model_identifier}' not explicitly supported by any provider. Falling back to DB default '{db_default_provider}'.")
                 try:
                     # Try to get provider config from database or gateway config
                     provider_config = self._get_provider_config(db_default_provider)
                     return db_default_provider
                 except ValueError:
                     logger.warning(f"DB default provider '{db_default_provider}' not configured. Trying gateway default.")
             
             # 6. Fallback to Default Provider from gateway config (if not excluded)
             ...
             if default_provider and default_provider != exclude_provider_id:
                  ...
                  try:
                      provider_config = self._get_provider_config(default_provider)
                      return default_provider
                  except ValueError as e:
                      logger.error(f"Default provider '{default_provider}' not found in config: {e}")
                      # Try to get the first available provider from the allowed_providers list
                      for provider_id in self.gateway_config.allowed_providers:
                          try:
                              provider_config = self._get_provider_config(provider_id)
                              logger.warning(f"Using '{provider_id}' as fallback for default provider")
                              return provider_id
                          except ValueError:
                              continue
                      raise RoutingError(f"Default provider '{default_provider}' not defined in config and no fallback found.")
             else:
                  raise RoutingError(f"No configured provider found for model '{model_identifier}' (excluding '{exclude_provider_id}').")


        # 7. Apply Routing Strategy (if multiple candidates)
